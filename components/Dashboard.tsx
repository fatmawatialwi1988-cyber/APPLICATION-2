...
import { Download, Plus, Trash2, FileSpreadsheet, Settings, Edit3, Info, User, RefreshCw, LogOut } from 'lucide-react';
import { TP, Student, Config, IntervalStatus } from '../types';
import { ConfigModal } from './ConfigModal';
import { TpEditorModal } from './TpEditorModal';
...
export const Dashboard: React.FC<DashboardProps> = ({ username, onLogout }) => {
  // Use username to create unique storage keys for this user
  const STORAGE_KEYS = {
    CONFIG: `aps_${username}_config`,
    TPS: `aps_${username}_tps`,
    STUDENTS: `aps_${username}_students`
  };
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
              <div className="text-right hidden sm:block mr-2">
                 <div className="flex flex-col items-end">
                    <span className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Logged in as</span>
                    <span className="text-yellow-400 font-bold flex items-center gap-1.5"><User size={14}/> {username}</span>
                 </div>
              </div>
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                  <button 
                    ...
                    className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold px-4 py-2.5 rounded-lg border border-slate-700 transition-all hover:shadow-lg hover:border-slate-500"
                  >
                    <Settings size={16} />
                  </button>
                  <button 
                    onClick={onLogout}
                    className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-red-900/30 hover:bg-red-900/50 text-red-200 text-xs font-bold px-4 py-2.5 rounded-lg border border-red-900/50 transition-all"
                    title="Log Out"
                  >
                    <LogOut size={16} />
                  </button>
        ...
        <div className="p-4 bg-white border-b border-slate-200 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 sticky top-0 z-20 shadow-sm">
          <div className="flex-1 w-full">
            <div className="flex items-center gap-2 text-slate-500 text-xs font-bold uppercase tracking-wider mb-3">
              ...
              <span className="ml-auto text-[10px] text-slate-400 flex items-center gap-1 lg:hidden">
                 {isSaved ? <span className="text-green-500">● Tersimpan</span> : <span className="text-yellow-500">● Menyimpan...</span>}
              </span>
            </div>
          ...
          <div className="flex-shrink-0 flex items-center gap-4">
             <div className="hidden lg:block text-[10px] text-right font-medium text-slate-400">
                {isSaved ? (
                  <span className="text-green-600 flex items-center gap-1"><RefreshCw size={10} /> Data Tersimpan Otomatis</span>
                ) : (
                  <span className="text-yellow-600 flex items-center gap-1">Menyimpan Perubahan...</span>
                )}
             </div>
            <button 
